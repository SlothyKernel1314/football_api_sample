...
def get_top_scorers_in_premier_league_gb_2018():
    league_id = "2"  # The {league_id} are unique in the API and represent a league/cup for a specific {season}
    url = "https://server1.api-football.com/topscorers/" + league_id
    response = requests.request("GET", url, headers=HEADERS)
    datas = response.json()
    print(datas)
...
def get_top_scorers_in_premier_league_gb_2019():
    league_id = "524"  # The {league_id} are unique in the API and represent a league/cup for a specific {season}
    url = "https://server1.api-football.com/topscorers/" + league_id
    response = requests.request("GET", url, headers=HEADERS)
    datas = response.json()
    print(datas)
